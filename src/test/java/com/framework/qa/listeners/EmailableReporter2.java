		...
		if (tests.getAllResults().size() > 0) {
			...
			for (ITestNGMethod method : getMethodSet(tests)) {
				...
				if (!className.equalsIgnoreCase(lastClassName)) {
					if (mq > 0) {
						cq += 1;
						m_out.println("<tr class=\"" + style + (cq % 2 == 0 ? "even" : "odd") + "\">" + "<td rowspan=\""
				...
				buff.append("<td><a href=\"#m" + m_methodIndex + "\">" + qualifiedName(method) + " "
						...
						+ "<td class=\"numi\">" + resultSet.size() + "</td><td class=\"numi\">" + (end - start)
						+ "</td></tr>");
			}
			if (mq > 0) {
				cq += 1;
				m_out.println("<tr class=\"" + style + (cq % 2 == 0 ? "even" : "odd") + "\">" + "<td rowspan=\"" + mq
						+ "\">" + lastClassName + buff);
			}
		}
	}
	...
	private void resultDetail(IResultMap tests, final String style) {
		if (tests.getAllResults().size() > 0) {
			...
			for (ITestNGMethod method : getMethodSet(tests)) {
				...
				m_out.println(
						"<a id=\"m" + m_methodIndex + "\"></a><h2>" + cname + ":" + method.getMethodName() + "</h2>");
				int rq = 0;
				Set<ITestResult> resultSet = tests.getResults(method);
				for (ITestResult ans : resultSet) {
					...
					if (hasParameters) {
						if (rq == 1) {
							...
							for (int x = 1; x <= parameters.length; x++) {
								m_out.print(
										"<th style=\"padding-left:1em;padding-right:1em\">Parameter #" + x + "</th>");
							}
							m_out.println("</tr>");
						}
					...
					if (hasReporterOutput || hasThrowable) {
						...
						if (hasParameters) {
							m_out.println("<tr" + (rq % 2 == 0 ? " class=\"stripe\"" : "") + "><td" + indent
